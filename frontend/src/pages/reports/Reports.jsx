        ...
        <div className="bg-white dark:bg-gray-800 rounded p-4">
          ...
          {monthly.length ? (
            <table className="w-full text-sm table-auto">
              ...
              <tbody>
                {monthly.map((m) => <tr key={m.label} className="border-t dark:border-gray-700"><td className="p-2 text-gray-900 dark:text-white">{m.label}</td><td className="p-2 text-gray-900 dark:text-white">{m.total}</td></tr>)}
              </tbody>
            </table>
        ...
        <div className="bg-white dark:bg-gray-800 rounded p-4">
          ...
          {trips.length ? (
            <table className="w-full text-sm table-auto">
              ...
              <tbody>
                {trips.map((t) => <tr key={t.status} className="border-t dark:border-gray-700"><td className="p-2 text-gray-900 dark:text-white">{t.status}</td><td className="p-2 text-gray-900 dark:text-white">{t.count}</td></tr>)}
              </tbody>
            </table>
          ) : <div className="text-sm text-gray-600 dark:text-gray-300">No data</div>}
        </div>
